...
class XTFPatchExtractor:
    ...
    def __init__(self):
        ...
        # Process XTFs 
        xtf_file_names = [file_name for file_name in os.listdir(config.XTF_FOLDER_PATH) if file_name.endswith(".xtf")]
    ...
    def process_xtf(self, xtf_file_name: str, 
        ...
        for i in range(0, num_pings, step):
            ...
            self.create_patches(swaths[ping_idx, port_idx], waterfall[ping_idx, port_idx], xtf_file_name, i, port_idx.start, True)
            self.create_patches(swaths[ping_idx, stbd_idx], waterfall[ping_idx, stbd_idx], xtf_file_name, i, stbd_idx.start, False)
    ...
    def create_patches(self, swaths: np.ndarray, 
        ...
        Args:
            ...
            waterfall (np.ndarray): Waterfall image of the XTF file 
            xtf_file_name (str): XTF file path 
            start_ping_idx (int): Starting row index of waterfall from which to extract sonar images 
            start_bin_idx (int): Starting column index of waterfall from which to extract sonar images 
        ...
        for row_idx in row_idxs:
            for col_idx in col_idxs:
                ...
                patch_file_name = str(self.patch_counter).zfill(len(str(max_patches))) + ".npy"
                ...
                metadata_row = [xtf_file_name, patch_file_path, 
                                ...
                                start_ping_idx + row_idx, start_ping_idx + row_idx + config.PATCH_HEIGHT, 
                                start_bin_idx + col_idx, start_bin_idx + col_idx + config.PATCH_WIDTH, config.EPSG_CODE,
    ...
    def plot_waterfall(self, waterfall: np.ndarray, 
        ...
        waterfall_img = waterfall[ping_idx,:] 
        ax[0].imshow(waterfall_img, cmap="gray")
        ax[0].set_title("Waterfall")

        port_img = waterfall[ping_idx,port_idx] 
        ax[1].imshow(port_img, cmap="gray")
        ax[1].set_title("Port")

        stbd_img = waterfall[ping_idx,stbd_idx] 
