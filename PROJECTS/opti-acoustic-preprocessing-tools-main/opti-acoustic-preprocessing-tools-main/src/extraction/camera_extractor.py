...
class CameraExtractor:
    ...
    def __init__(self):
        ...
        # Process ROS Bags 
        bag_file_names = [file_name for file_name in os.listdir(config.ROSBAG_FOLDER_PATH) if file_name.endswith(".bag")]
    ...
    def process_bag(self, reader: BagReader, 
        ...
        for i, topic in enumerate(camera_topics):
            try:
                topic_calib = self.calib_data[reader.get_bag_file_name()][topic]
