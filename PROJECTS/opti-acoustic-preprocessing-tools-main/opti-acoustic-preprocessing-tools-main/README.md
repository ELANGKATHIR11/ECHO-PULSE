# Opti-acoustic Pre-processing Tools

# 1. Description

**Authors**: Valerio Franchi and Hayat Rajani

This repository contains several pre-processing tools developed for the opti-acoustic datasets created as part of the work *"BenthiCat: An opti-acoustic dataset for advancing benthic classification and habitat mapping"* by Rajani et al., currently under review by *The International Journal of Robotics Research* (IJRR). A preprint of the manuscript can be found on [arXiv](https://arxiv.org/abs/2510.04876), and the corresponding datasets are hosted on the [Harvard Dataverse](https://dataverse.harvard.edu/dataverse/benthicat).
...
- Opti-acoustic dataset construction from camera and sonar images  
...
```
git clone `https://github.com/CIRS-Girona/opti-acoustic-preprocessing-tools`
```
...
```
cd opti-acoustic-preprocessing-tools
...
- **XTF_RPY_RAD**: boolean variable describing the unit of the angles inside the XTF files (radians or degrees)
- **XTF_UTC_TIME**: time zone difference from UTC+0 of the collected data inside the XTF files 
- **XTF_INTENSITY_BITS**: number of bits for each intensity value inside the XTF files 

### Image parameters 

- **MIN_IMAGE_HEIGHT**: minimum altitude for image extraction
